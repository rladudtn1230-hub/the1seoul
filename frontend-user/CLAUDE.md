...
7. **SEO/웹 접근성을 위해 시멘틱 태그를 사용**한다. 의미 없는 `<div>` 남용 대신 `<header>`, `<nav>`, `<main>`, `<footer>`, `<section>`, `<article>`, `<aside>` 등 문맥에 맞는 태그를 쓴다. 페이지당 `<h1>`은 하나, 제목 레벨은 건너뛰지 않고 순서대로 중첩한다. 클릭 가능한 요소는 `<div onClick>`이 아니라 `<button>`/`<a>`(`next/link` 래퍼)를 사용하고, 아이콘 전용 버튼 등에는 `aria-label`을 붙인다. 이미지는 의미 있는 `alt`를 채운다.
